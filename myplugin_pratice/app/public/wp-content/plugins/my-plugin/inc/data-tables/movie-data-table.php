 ...
 */
function output_column_content($columns,$post_id){
    switch($columns){
        case 'image':
            echo get_the_post_thumbnail($post_id,'poster-thumbnail');
            break;
        case 'release_year':
            echo get_post_meta($post_id,'_myplugin_movie_releases_year',true);
            break;
        case 'video_type':
...
function myplugin_make_movie_colmns_sortable($columns){
    $columns['release_year']='release_year';
    $columns['video_type']='video_type';

    return $columns;
}
add_filter('manage_edit-movie_sortable_columns','myplugin_make_movie_colmns_sortable');
...
function myplugin_movie_columns_sorting_logic($query){
    ...
    if(!is_admin() || ! $query->is_main_uery()){
        return;
    }
